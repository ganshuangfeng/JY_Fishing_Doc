return {
	active_award_config=
	{
		[1]=
		{
			id = 1,
			active = 30,
			award_tip = "2万金币，锁定*2",
		},
		[2]=
		{
			id = 2,
			active = 60,
			award_tip = "5万金币，10福利券",
		},
		[3]=
		{
			id = 3,
			active = 90,
			award_tip = "5万金币，召唤*3",
		},
		[4]=
		{
			id = 4,
			active = 120,
			award_tip = "10万金币，20福利券",
		},
		[5]=
		{
			id = 5,
			active = 150,
			award_tip = "10万金币，10万鱼币",
		},
	},
	active_task_item=
	{
		[1]=
		{
			id = 1,
			task_id = 12512,
			task_name = "进入游戏",
			task_icon = "rw_icon_hd",
			task_instruction = "进入龙王宝藏任意场1次",
			task_award_icon = {"ty_icon_jb_15y","ty_icon_hytb",},
			task_award_instruction = {"金币","活跃度"},
			task_award_count = {"x100","x10"},
			gotoui = {"game_Fishing3DHall",},
		},
		[2]=
		{
			id = 2,
			task_id = 12520,
			task_name = "使用技能",
			task_icon = "rw_icon_jn",
			task_instruction = "龙王宝藏中使用任意技能3次",
			task_award_icon = {"ty_icon_jb_30y","ty_icon_hytb",},
			task_award_instruction = {"金币","活跃度"},
			task_award_count = {"x5000","x20"},
			gotoui = {"game_Fishing3DHall",},
		},
		[3]=
		{
			id = 3,
			task_id = 12521,
			task_name = "累计捕鱼",
			task_icon = "rw_icon_bh",
			task_instruction = "龙王宝藏中捕获任意10条鱼",
			task_award_icon = {"ty_icon_jb_30y","ty_icon_hytb",},
			task_award_instruction = {"金币","活跃度"},
			task_award_count = {"x5000","x20"},
			gotoui = {"game_Fishing3DHall",},
		},
		[4]=
		{
			id = 4,
			task_id = 12522,
			task_name = "累计开炮",
			task_icon = "rw_icon_ljpd",
			task_instruction = "龙王宝藏中累计开炮1000次",
			task_award_icon = {"ty_icon_jb_50y","ty_icon_hytb",},
			task_award_instruction = {"金币","活跃度"},
			task_award_count = {"x1万","x30"},
			gotoui = {"game_Fishing3DHall",},
		},
		[5]=
		{
			id = 5,
			task_id = 12523,
			task_name = "累计在线",
			task_icon = "rw_icon_ljsj",
			task_instruction = "龙王宝藏中累计在线90分钟",
			task_award_icon = {"ty_icon_flq1","ty_icon_hytb",},
			task_award_instruction = {"福利券","活跃度"},
			task_award_count = {"x30","x30"},
			gotoui = {"game_Fishing3DHall",},
		},
		[6]=
		{
			id = 6,
			task_id = 12516,
			task_name = "捕获彩金",
			task_icon = "rw_icon_js",
			task_instruction = "龙王宝藏中捕获3条彩金鱼",
			task_award_icon = {"ty_icon_jb_15y","ty_icon_hytb",},
			task_award_instruction = {"金币","活跃度"},
			task_award_count = {"x1万","x30"},
			gotoui = {"game_Fishing3DHall",},
		},
		[7]=
		{
			id = 7,
			task_id = 12519,
			task_name = "单笔充值",
			task_icon = "rw_icon_mrzz",
			task_instruction = "单笔充值6元及以上",
			task_award_icon = {"ty_icon_flq3","ty_icon_hytb",},
			task_award_instruction = {"福利券","活跃度"},
			task_award_count = {"x30","x50"},
			gotoui = {"shop_bay","jing_bi"},
		},
	},
}